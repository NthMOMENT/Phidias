"""Phidias v1.1 — integrated agent: Telegram bot + payment monitor + pipeline worker.
Photo in -> order -> USDC payment on Solana devnet -> STL out. No human in the loop.
Run: python src/phidias_bot.py  (inside tmux)"""
import asyncio, json, os, random, subprocess, logging, time
    ...
    while True:
        try:
            ...
            for m, od in orders.items():
                ...
                mode = od["mode"]
                cmd = ["/home/phidias/phidias/venv/bin/python", "src/relief2.py", od["image"],
...
log.info("Phidias v1.1 starting")
app.run_polling()
